<template>
    <div>
        <div v-if="!loading" class="weather container">
            <p class="app__date has-text-weight-bold">{{ new Date(time).toDateString() + " " + new Date(time).toLocaleTimeString() }}</p>
...
export default {
    ...
    props: ['id'],
    computed: {
        ...mapGetters(['location', 'time', 'weatherDescription', 'imageAbbr', 'weatherTemp', 'loading'])
    },
    watch: {
        id(){
            this.fetchWeather();
